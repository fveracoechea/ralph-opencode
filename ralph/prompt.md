...
2. Read the progress log at `ralph/progress.txt` (check Codebase Patterns
   section first)
...
1. **Identify directories with edited files** - Look at which directories you
   modified
2. **Check for existing AGENTS.md** - Look for AGENTS.md in those directories or
   parent directories
...
2. Navigate to the relevant page at `http://localhost:5173` or
   `http://localhost:3000` (depending on the dev server)
